...
class shareMe {
	...
	UNIQUE KEY id (id)
        ...
        $wpdb->query($wpdb->prepare("INSERT INTO $table (`id`, `name`, `status`)VALUES ( %d, %s, %d )", 8, 'mail', 1));
	...
	UNIQUE KEY id (id)
        ...
        update_option('sm_auto_display', '1');
    }
     ...
     * @return type
     */
    public static function sm_getShareMe() {
        global $wpdb;
        $sm_theme = "circle";
        $sm_h_pos = "right";
        $sm_v_pos = "up";
    ...
    public static function sm_getLink($type) {
        ...
        switch ($type) {
            case 'facebook': $link = '<a href="http://www.facebook.com/sharer.php?u=' . apply_filters("the_permalink", get_permalink()) . '&t=' . urlencode(get_the_title()) . '" alt="' . __("Share on Facebook") . '" title=""   onclick="return smWindowpop(this.href,' . self::$popinWidth . ',' . self:: $popinHight . ')">';
                break;
            case 'twitter': $link = '<a href="http://twitter.com/share?text=' . urlencode(get_the_title()) . '-&url=' . apply_filters("the_permalink", get_permalink()) . '&via=' . $blogName . '" alt="' . __("Tweet This Post") . '" title="' . __("Tweet This Post") . '"  onclick="return smWindowpop(this.href,' . self::$popinWidth . ',' . self::$popinHight . ')">';
                break;
            case 'googleplus': $link = '<a href="https://plusone.google.com/_/+1/confirm?hl=fr-FR&url=' . apply_filters("the_permalink", get_permalink()) . '" alt="' . __("Share on Google+") . '" title="' . __("Share on Google+") . '"  target="_blank" onclick="return smWindowpop(this.href,' . self::$popinWidth . ',' . self::$popinHight . ')">';
                break;
            case 'tumblr': $thumbID = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'large');
                $link = '<a href="http://www.tumblr.com/share/photo?source=' . urlencode($thumbID[0]) . '&caption=' . urlencode(get_the_title()) . '&clickthru=' . urlencode(get_permalink()) . '" alt="' . __("Share on Tumblr") . '"  title="' . __("Share on Tumblr") . '"  onclick="return smWindowpop(this.href,' . self::$popinWidth . ',' . self::$popinHight . ')"> ';
                break;
            case 'linkedin': $link = '<a href="http://www.linkedin.com/shareArticle?mini=true&url=' . apply_filters("the_permalink", get_permalink()) . '&title=' . urlencode(get_the_title()) . '&source=' . $blogName . '"  alt="' . __("Share on Linkedin") . '" title="' . __("Share on Linkedin") . '"onclick="return smWindowpop(this.href, ' . self::$popinWidth . ',' . self::$popinHight . ')">';
                break;
            case 'blogger': $link = '<a href="https://www.blogger.com/blog-this.g?u=' . apply_filters("the_permalink", get_permalink()) . '&n=' . urlencode(get_the_title()) . '&t=' . $blogName . '" alt="' . __("Share on Blogger") . '" title="' . __("Share on Blogger") . '" onclick="return smWindowpop(this.href, ' . self::$popinWidth . ',' . self::$popinHight . ')">';
                break;
            case 'mail': $link = '<a href="mailto:?subject=' . get_the_title() . '&Body=' . __("Link") . '   %0D ' . apply_filters("the_permalink", get_permalink()) . ' ">';
                break;
            default: $link = '';
                break;
        }
